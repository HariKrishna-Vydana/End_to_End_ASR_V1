...
class decoder(nn.Module):
        def __init__(self, args):
                ...
                #---------------------------------------  
                if self.use_speller:         
                    self.Spelling_Lstm  = nn.LSTM(self.hidden_size,self.hidden_size, 1 ,batch_first=False,bidirectional=False)#1
        ...
        #----------------------------------------------------------------------------------
        #----------------------------------------------------------------------------------
        def Spelling_step(self,H,yi,hn1,cn1,si,alpha_i_prev): #### This is just an alternative word LSTM but does not look in to encoder
                ...
                return yout,alpha_i,si_out,hn1_out,cn1_out
        #=======================================================================================
        #-------------------------------------------
        def Spell_Rnn(self, UNK_TEXT, H, hn1, cn1, si, alpha_i_prev,greedy_label):
                batch_size=H.size(1)
                #import pdb; pdb.set_trace()

                char_seq_list     = [self.Char_model.EncodeAsIds(i) for i in UNK_TEXT if i.strip()]
                char_seq_len_list = [len(self.Char_model.EncodeAsIds(i)) for i in UNK_TEXT if i.strip()]  

                UNK_CHAR_SEQ = pad_sequences(char_seq_list, maxlen=max(char_seq_len_list) ,dtype='int32',padding='post',value=self.Ch_SIL_tok)
                ...
                Unk_emb_vec     = self.Wembedding(torch.tensor([[self.word_unk]]))
                Unk_emb_vec     = Unk_emb_vec.cuda() if self.use_gpu else Unk_emb_vec
                Rep_Unk_emb_vec = torch.repeat_interleave(Unk_emb_vec,batch_size, dim=1)
                yi_c = torch.repeat_interleave(Rep_Unk_emb_vec, Target_UNK_CHAR_SEQ.size(1), dim=0)
                
                #import pdb; pdb.set_trace()
                char_yout , alpha_i, si_out_char, hn1_out, cn1_out = self.Spelling_step(H, yi_c, hn1, cn1, si, alpha_i_prev)
                char_yout    = torch.transpose(char_yout, 0, 1)
                char_yout    = char_yout.contiguous().view(-1,self.Char_out_size)
                

                #import pdb; pdb.set_trace()
                utt_with_unk = (greedy_label != self.word_unk)
                
                #### just while computing the loss for only the senteces with 
                #### word unk token just replace the target token with self.Ch_SIL_tok
                if not self.spell_loss_perbatch:
                    Target_UNK_CHAR_SEQ[utt_with_unk.squeeze(0) ] = self.Ch_SIL_tok
                Target_UNK_CHAR_SEQ = Target_UNK_CHAR_SEQ.contiguous().view(-1)


                #import pdb; pdb.set_trace()
                char_lstm_loss = cal_loss(char_yout, Target_UNK_CHAR_SEQ, self.Ch_SIL_tok, normalize_length=True, smoothing=self.label_smoothing)
                utt_with_unk   = utt_with_unk.unsqueeze(2).expand_as(hn1)*1

                #import pdb; pdb.set_trace()
                hn1_out = hn1 * utt_with_unk + (1-utt_with_unk) * hn1_out
                cn1_out = cn1 * utt_with_unk + (1-utt_with_unk) * cn1_out
                return char_lstm_loss, char_yout, alpha_i, si_out_char, hn1_out, cn1_out
        #---------------------------------------------------------------------------------------
        #---------------------------------------------------------------------------------------
        ...
        def forward(self,H,teacher_force_rate,Char_target,Word_target,L_text):
                ...
                for d_steps in range(decoder_steps):
                        ...
                        if self.use_speller:
                            ...
                            if torch.any(greedy_label==self.word_unk):
                                UNK_TEXT=L_text[:,d_steps]
                                char_lstm_loss, char_yout, alpha_i, si_out_char, hn1_out, cn1_out,ci_out = self.Spell_Rnn(UNK_TEXT, H, hn1, cn1, si, alpha_i_prev, ci, greedy_label)
                        #-------------------------------------------- 
                        ...
                        cost += present_word_cost + char_lstm_loss
                        #-----------------------------------------
